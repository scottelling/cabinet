...
test("direct API adapter executes file tools inside the active cabinet", async () => {
  ...
  try {
    ...
    assert.ok(observedToolNames.includes("browse_web"));
...
test("direct API agents can read the public web and return visible browser evidence", async () => {
  const adapter = directApiAdapters.find((entry) => entry.providerId === "openai-api");
  assert.ok(adapter?.execute);

  const cwd = path.join(DATA_DIR, "direct-api-browser-room");
  await fs.rm(cwd, { recursive: true, force: true });
  await fs.mkdir(cwd, { recursive: true });
  ...
  const previous = {
    ...
    browserToken: process.env.CLOUDFLARE_BROWSER_RUN_API_TOKEN,
  };
  process.env.OPENAI_API_KEY = "sk-test-only";
  process.env.CLOUDFLARE_ACCOUNT_ID = "cf-account-test";
  process.env.CLOUDFLARE_BROWSER_RUN_API_TOKEN = "cf-browser-test";
  let modelCalls = 0;
  let browserCalls = 0;
  ...
  globalThis.fetch = (async (input) => {
    ...
    if (url.includes("api.cloudflare.com")) {
      browserCalls += 1;
      return new Response(
        JSON.stringify({ success: true, result: "# Current page\n\nVerified today." }),
        {
          status: 200,
          headers: {
            "content-type": "application/json",
            "x-browser-ms-used": "88",
          },
        },
      );
    }
        ...
        choices: [
          {
            message: {
              role: "assistant",
              content: "The page is current: https://example.com/current",
            },
          },
        ],
      }),
  ...
  try {
    ...
    assert.equal(result.browserEvidence?.[0].engine, "kitesurf");
    assert.match(result.output || "", /Browser evidence used:/);
    assert.match(logs.join(""), /\[Browser\] read https:\/\/example\.com\/current/);
  } finally {
    globalThis.fetch = originalFetch;
    if (previous.openai === undefined) delete process.env.OPENAI_API_KEY;
    else process.env.OPENAI_API_KEY = previous.openai;
    if (previous.account === undefined) delete process.env.CLOUDFLARE_ACCOUNT_ID;
    else process.env.CLOUDFLARE_ACCOUNT_ID = previous.account;
    if (previous.browserToken === undefined) {
      delete process.env.CLOUDFLARE_BROWSER_RUN_API_TOKEN;
    } else {
      process.env.CLOUDFLARE_BROWSER_RUN_API_TOKEN = previous.browserToken;
    }
    await fs.rm(cwd, { recursive: true, force: true });
  }
});
